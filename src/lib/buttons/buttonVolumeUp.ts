...
export async function buttonVolumeUp(interaction: ButtonInteraction, kazagumo: Kazagumo) {
  ...
  const msg = await interaction.reply({ content: `🔊 Volume increased to **${newVolume}%**.` });
  setTimeout(() => msg.delete().catch(() => {}), MSGDEL_DELAY);
}
